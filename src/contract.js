...
class Contract {
  ...
  async getFromContractWithParams(functionInfo) {
    const contractQuery = await new ContractCallQuery({
      gas: 100000,
      contractId: process.env.REACT_APP_CONTRACT_ID,
    })
      .setFunction(functionInfo.name, functionInfo.parameters)
      .execute(this.client);
    return contractQuery;
  }
  ...
  async setIntoContractWithParams(functionInfo) {
    const contractExecTx = await new ContractExecuteTransaction({
      contractId: process.env.REACT_APP_CONTRACT_ID,
      gas: 100000,
      function: functionInfo,
